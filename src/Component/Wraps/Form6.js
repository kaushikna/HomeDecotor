...
const Form6 = ({ userMobile, setFormNow, setmessageLabel, setLoginRes, expId }) => {
  ...
  const submitOTPForm = async(e)=>{
    ...
    if(OTP && userMobile){
      ...
      let responseObj = await verifyOTP({phone:userMobile,otpLogin:'true',otp:OTP,pronto_login:'true',ajax_mode:'1',pronto_source:"Pronto Paint",first_interaction:`|ExperimentID=${expId}`});
  ...
  return (
    <div className="new-form" >
        ...
        <div >
          <h3 className="form-head"><span className="backArrow"> <img src="/images/left.png" onClick={e=>{e.preventDefault();setFormNow(0)}}></img></span>{" "}Talk to a colour consultant</h3>
